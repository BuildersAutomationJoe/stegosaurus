/* eslint-disable */
// this is an auto generated file. This will be overwritten

export const onCreateUserProfile = /* GraphQL */ `
  subscription OnCreateUserProfile(
    $filter: ModelSubscriptionUserProfileFilterInput
  ) {
    onCreateUserProfile(filter: $filter) {
      id
      email
      createdAt
      updatedAt
      __typename
    }
  }
`;
export const onUpdateUserProfile = /* GraphQL */ `
  subscription OnUpdateUserProfile(
    $filter: ModelSubscriptionUserProfileFilterInput
  ) {
    onUpdateUserProfile(filter: $filter) {
      id
      email
      createdAt
      updatedAt
      __typename
    }
  }
`;
export const onDeleteUserProfile = /* GraphQL */ `
  subscription OnDeleteUserProfile(
    $filter: ModelSubscriptionUserProfileFilterInput
  ) {
    onDeleteUserProfile(filter: $filter) {
      id
      email
      createdAt
      updatedAt
      __typename
    }
  }
`;
export const onCreateTextInputs = /* GraphQL */ `
  subscription OnCreateTextInputs(
    $filter: ModelSubscriptionTextInputsFilterInput
  ) {
    onCreateTextInputs(filter: $filter) {
      id
      text
      createdAt
      updatedAt
      __typename
    }
  }
`;
export const onUpdateTextInputs = /* GraphQL */ `
  subscription OnUpdateTextInputs(
    $filter: ModelSubscriptionTextInputsFilterInput
  ) {
    onUpdateTextInputs(filter: $filter) {
      id
      text
      createdAt
      updatedAt
      __typename
    }
  }
`;
export const onDeleteTextInputs = /* GraphQL */ `
  subscription OnDeleteTextInputs(
    $filter: ModelSubscriptionTextInputsFilterInput
  ) {
    onDeleteTextInputs(filter: $filter) {
      id
      text
      createdAt
      updatedAt
      __typename
    }
  }
`;
export const onCreateDataObject = /* GraphQL */ `
  subscription OnCreateDataObject(
    $filter: ModelSubscriptionDataObjectFilterInput
  ) {
    onCreateDataObject(filter: $filter) {
      id
      filename
      createdAt
      updatedAt
      __typename
    }
  }
`;
export const onUpdateDataObject = /* GraphQL */ `
  subscription OnUpdateDataObject(
    $filter: ModelSubscriptionDataObjectFilterInput
  ) {
    onUpdateDataObject(filter: $filter) {
      id
      filename
      createdAt
      updatedAt
      __typename
    }
  }
`;
export const onDeleteDataObject = /* GraphQL */ `
  subscription OnDeleteDataObject(
    $filter: ModelSubscriptionDataObjectFilterInput
  ) {
    onDeleteDataObject(filter: $filter) {
      id
      filename
      createdAt
      updatedAt
      __typename
    }
  }
`;
